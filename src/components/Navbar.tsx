        ...
        <div className="flex items-center gap-4 sm:gap-6">
          ...
          {onOpenAuthModal && (
            <button
              ...
              className="px-3.5 py-1.5 rounded-full border border-slate-300 hover:border-slate-400 bg-white hover:bg-slate-50 text-slate-700 text-xs font-semibold flex items-center gap-2 transition-all cursor-pointer shadow-xs"
              title={googleUser ? `Signed in as ${googleUser.email}` : 'Sign in with Google'}
            >
              {googleUser ? (
                <>
                  <img src={googleUser.picture} alt={googleUser.name} className="w-5 h-5 rounded-full border border-emerald-500" />
                  <span className="hidden sm:inline font-bold text-slate-800">{googleUser.name.split(' ')[0]}</span>
